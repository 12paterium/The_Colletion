...
async def generate_name(session, image_path):
...
async def process_image(session, sem, fname):
    async with sem:
        old_path = os.path.join(IMAGE_DIR, fname)

        try:
            desc = await generate_name(session, old_path)
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [process_image(session, sem, f) for f in files]
